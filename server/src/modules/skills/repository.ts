 ...
 * the agent side (link/reorder/list for an agent); this repository never
 * touches `agent_skills`. Workspace-scoped throughout.
 */
...
export class SkillsRepository {
  ...
  /** Delete a skill (scoped to workspace). Versions/agent-links cascade via FK. */
  ...
  /**
   * Update a skill. `bumpVersion` (whether this patch changes the body/name/
   * type vs. just toggling `enabled`) is decided by the service — the
   * repository only persists the decision and snapshots skill_versions when
   * true.
   */
  async update(
    workspaceId: string,
